...
static int setup_lcd(void)
{
	...
	/* Drive backlight by PWM1 instead of forcing GPIO high. */
	if (!IS_ENABLED(CONFIG_DM_PWM)) {
		printf("PWM backlight requires CONFIG_DM_PWM\n");
		return -ENOSYS;
	}

	ret = uclass_get_device_by_seq(UCLASS_PWM, 0, &pwm);
	if (ret) {
		printf("Cannot get PWM device: %d\n", ret);
		return ret;
	}

	/* DTS uses 500000ns period; set 80%% duty to turn backlight on. */
	ret = pwm_set_config(pwm, 0, 500000, 400000);
	if (ret) {
		printf("Cannot configure PWM backlight: %d\n", ret);
		return ret;
	}

	ret = pwm_set_enable(pwm, 0, true);
	if (ret) {
		printf("Cannot enable PWM backlight: %d\n", ret);
		return ret;
	}
